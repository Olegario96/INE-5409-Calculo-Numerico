function c=f_coef_tchebychev(n, lim_inferior, lim_superior)
    ...
    soma=0;
    for k=1:m
        soma=soma+f_analisada(t(k), lim_inferior, lim_superior)*(1);
    end
    %c(1)=(1/pi)*(pi/m*soma);
    c(i+1)=1/m*soma;
    ...
    for k=1:m
        T(i,k)=t(k); %T1=t
        soma=soma+f_analisada(t(k), lim_inferior, lim_superior)*T(i,k);
    end
    %c(2)=(2/pi)*(pi/m*soma);
    c(i+1)=2/m*soma;
